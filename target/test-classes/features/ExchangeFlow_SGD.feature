@exchange @sgd @offer
Feature: End to End Exchange flow - SGD

  Background:
    Given I setup test data file name for "ForeignExchange" "SGD" "190"
  ...
  Scenario Outline: Verify the end to end flow of Exchange flow for AUD corridor
    ...
    When I select "Next business day" tenor on Exchange page
...
#    Then I search for the transaction with "<reference>", "<type>" and "<status>"
    ...
    Examples:
      ...
      | SGD         | 190.00     | Instant        | +91 7989730020 | Foreign Exchange |                      |